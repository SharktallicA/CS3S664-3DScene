...
void BlurUtility::blurModel(Model*obj, ID3D11ShaderResourceView	*depthSRV)
{
	...
	if (obj) {
		...
		//ID3D11BlendState *objBlendState = obj->getEffect()->getBlendState();
		//ID3D11DepthStencilState *objDSState = obj->getEffect()->getDepthStencilState();
		//obj->getEffect()->setBlendState(defaultEffect->getBlendState());
		//obj->getEffect()->setBlendState(alphaOnBlendState);
		obj->render(context);
		///obj->getEffect()->setBlendState(objBlendState);
	//	obj->getEffect()->setDepthStencilState(objDSState);
